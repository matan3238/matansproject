...
🙏 תודות
תודה מיוחדת ל-Matan BenAbu (matan3238) על העיצוב וה-UI של האתר – ממשק המשתמש, העיצוב המודרני והחוויה הוויזואלית.
...
איך הכל מתנהל
זרימת לקוח
גלישה – דף בית או דף מוצרים, סינון לפי קטגוריה/מותג/אחסון
בחירת מוצר – לחיצה על מוצר → דף פרטי מוצר (בחירת צבע/אחסון אם קיים)
הוספה לסל – דורש התחברות. המוצר נשמר בסל של המשתמש
סל קניות – צפייה, עדכון כמויות, הסרה
תשלום – "המשך לתשלום" → דף תשלום (טופס כרטיס) → יצירת הזמנה
אחרי תשלום – מעבר לפרופיל, הזמנה חדשה בהיסטוריה
זרימת מנהל
התחברות – עם משתמש מנהל (admin@matan.com / 123456)
לוח בקרה – קישור בתפריט או גישה ל-admin.html
ניהול – מוצרים (הוספה/עריכה/מחיקה), הזמנות (צפייה/עריכת סטטוס), משתמשים (הוספה/עריכה/מחיקה), קטגוריות (הוספה/מחיקה)
אחסון נתונים
רכיב	אחסון
...
תודה ל-Matan BenAbu על העיצוב וה-UI המעולה! 🎨
